...
Character I/O functions are specialized tools in C that handle input and output one character at a time. These functions provide more granular control compared to their formatted counterparts like printf() and scanf(). The main functions we'll explore are:
...
1. **Q: Why use int instead of char for getchar()?**
   A: getchar() can return EOF (-1), which might not fit in a char variable.

2. **Q: How can I clear the input buffer?**
   A: Use a while loop with getchar() until you reach a newline character.

3. **Q: Why doesn't my getchar() program continue immediately after I type?**
   A: getchar() waits for Enter due to buffered input.

4. **Q: Is putchar() faster than printf()?**
   A: Yes, putchar() is generally faster for single-character output.

5. **Q: Can I use these functions for file I/O?**
   A: Yes, but you'll need to redirect standard input/output or use file-specific variants.
