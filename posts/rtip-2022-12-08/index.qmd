...
date: "2022-12-08"
categories: [code, rtip, histogram, healthyrai]
---
...
hai_histogram_facet_plot(mtcars, .interactive = TRUE)
```

Now lets scale the data and review.

```{r ex2}
hai_histogram_facet_plot(mtcars, .scale_data = TRUE, .interactive = TRUE)
```

Let's take a look the iris data set now.

```{r ex3}
hai_histogram_facet_plot(iris, .interactive = TRUE)
```
