...
1. Q: Which method is fastest for large datasets?
   A: data.table consistently outperforms other methods for large datasets.

2. Q: Can these methods handle missing values?
   A: Yes, all methods can handle NA values, but you may need to specify na.rm = TRUE.

3. Q: How do I count duplicates across multiple columns?
   A: Use group_by() with multiple columns in dplyr or multiple columns in data.table's by parameter.

4. Q: Will these methods work with character vectors?
   A: Yes, all methods work with character, numeric, and factor data types.

5. Q: How can I improve performance when working with millions of rows?
   A: Use data.table and consider indexing frequently used columns.
